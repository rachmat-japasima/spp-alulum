<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Laporan Transaksi Pembayaran Tahun Ajaran') }}
        </h2>
    </x-slot>
    ...
    <div class="content">
        <div class="row">
            <div class="col-12">
                <h2 class="content-title">{{ __('Laporan Transaksi Pembayaran Tahun Ajaran') }}</h2>
                <h5 class="content-desc mb-4">
                    {{ __('Laporan transaksi pembayaran tahun ajaran untuk aplikasi SPP Al Ulum.') }}</h5>
            </div>
            @if ($errors->any())
                ...
                <div class="statistics-card card">
                        ...
                        <div class="row g-3 align-items-center">
                            ...
                            <div class="col-3">
                                <select class="form-select" name="date" id="date" required>
                                    @foreach ($listSchoolYear as $item)
                                        <option value="{{ $item->tahun_ajaran }}"
                                            {{ old('date', $filters->date) == $item->tahun_ajaran ? 'selected' : '' }}>
                    ...
                    <div class="col-auto d-flex justify-content-end">
                        <form action="{{ route('reports.printResume') }}" method="POST" target="_blank">
                            @csrf
                            <input type="hidden" name="type" value="schoolYear">
        ...
        <div class="col-12">
            <div class="statistics-card card">
                <h5 class="content-desc">
                    Rincian Transaksi Tahun Ajaran
                </h5>
    ...
    @push('addon-scripts')
        ...
        <script>
            $(document).ready(function() {
                ...
                $.ajax({
                    method: "POST",
                    url: "{{ route('reports.getDiagramDataSchoolYear') }}",
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                    },
                    data: {
                    ...
                    Highcharts.chart('lineChart', {
                        title: {
                            text: 'Laporan Transaksi Pembayaran Tahun Ajaran',
                            align: 'left'
                        },
                        subtitle: {
                        ...
                        tooltip: {
                            formatter: function() {
                                const cat = this.series.chart.xAxis[0].categories[this.x];
                                return `
        <span style="color:${this.color}">\u25CF</span> <b>${this.series.name}</b><br/>
        Total (Rp.): <b>${rupiah(this.y)}</b><br/>
        Bulan: <b>${cat}</b><br/>
      `;
                            }
                        },
                        ...
                        colors: ['#00DFA2', '#D80032', '#3085C3', '#C4DFDF'],
