<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Laporan Transaksi Pembayaran Per Semester') }}
        </h2>
    </x-slot>
    ...
    <div class="content">
        <div class="row">
            <div class="col-12">
                <h2 class="content-title">{{ __('Laporan Transaksi Pembayaran Per Semester') }}</h2>
                <h5 class="content-desc mb-4">
                    {{ __('Laporan transaksi pembayaran per semester untuk aplikasi SPP Al Ulum.') }}</h5>
            </div>
            @if ($errors->any())
            ...
            <div class="col-12">
                <div class="statistics-card card">
                    <figure>
                        ...
                        <p class="text-center">Data Semester {{ $filters->semester }} {{ $filters->date }}</p>
                    </figure>
                </div>
            </div>
            ...
            <div class="col-6 col-md-3 mt-3">
                <div class="card radius-10 bg-primary border-0 border-3">
                    <div class="card-body">
                        <div class="d-flex align-items-center">
                            <div>
                                <p class="mb-0 text-white">Total Uang Pemeliharaan & Pengembangan</p>
                                <h4 class="my-1 text-white">@currency($total_upp)</h4>
                                <p class="mb-0 font-13"></p>
                            </div>
                            <div class="widgets-icons-2 rounded-circle bg-gradient-blooker text-white ms-auto">
                                <i class="fa fa-users"></i>
            ...
            <div class="col-6 col-md-3 mt-3">
                <div class="card radius-10 bg-warning border-0 border-3">
                    <div class="card-body">
                        <div class="d-flex align-items-center">
                            <div>
                                <p class="mb-0 text-dark">Potongan Uang Pemeliharaan & Pengembangan</p>
                                <h4 class="my-1 text-dark">@currency($total_potongan_upp)</h4>
                                <p class="mb-0 font-13"></p>
                            </div>
                            <div class="widgets-icons-2 rounded-circle bg-gradient-blooker text-white ms-auto">
                                <i class="fa fa-users"></i>
            ...
            <div class="col-6 col-md-3 mt-3">
                <div class="card radius-10 bg-info border-0 border-3">
                    <div class="card-body">
                        <div class="d-flex align-items-center">
                            <div>
                                <p class="mb-0 text-white">Total Uang Perlengkapan</p>
                                <h4 class="my-1 text-white">@currency($total_upk)</h4>
                                <p class="mb-0 font-13"></p>
                            </div>
                            <div class="widgets-icons-2 rounded-circle bg-gradient-blooker text-white ms-auto">
                                <i class="fa fa-users"></i>
                        ...
                        <div class="row g-3 align-items-center">
                            ...
                            <div class="col-2">
                                <select class="form-select" name="tingkat" id="tingkat" required>
                                    <option value="full"
                                        {{ old('tingkat', $filters->tingkat) == 'full' ? 'selected' : '' }}>Semua
                                    </option>
                                    <option value="RA"
                                        {{ old('tingkat', $filters->tingkat) == 'RA' ? 'selected' : '' }}>RA</option>
                                    <option value="0"
                                        {{ old('tingkat', $filters->tingkat) == '0' ? 'selected' : '' }}>SD</option>
                                    <option value="1"
                                        {{ old('tingkat', $filters->tingkat) == '1' ? 'selected' : '' }}>SMP</option>
                                    <option value="2"
                                        {{ old('tingkat', $filters->tingkat) == '2' ? 'selected' : '' }}>SMA</option>
                                </select>
                            ...
                            <div class="col-2">
                                <select class="form-select" name="semester" id="semester" required>
                                    <option value="Ganjil"
                                        {{ old('semester', $filters->semester) == 'Ganjil' ? 'selected' : '' }}>Ganjil
                                    </option>
                                    <option value="Genap"
                                        {{ old('semester', $filters->semester) == 'Genap' ? 'selected' : '' }}>Genap
                                    </option>
                                </select>
                                <x-input-error class="mt-2 text-danger" :messages="$errors->get('semester')" />
                            </div>
                            <div class="col-2">
                                <select class="form-select" name="date" id="date" required>
                                    @for ($i = 2008; $i <= date('Y'); $i++)
                                        <option value="{{ $i }}"
                                            {{ old('date', $filters->date) == $i ? 'selected' : '' }}>
                                            {{ $i }}</option>
                                    @endfor
                                </select>
                            ...
                            <div class="col-auto">
                                <button type="submit" class="btn btn-form me-auto">
                                    <img src="{{ url('/assets/img/global/filter.svg') }}" width="20"
                                        alt="">
                                    Filter
                                </button>
                            </div>
        ...
        <div class="col-12">
            <div class="statistics-card card">
                <h5 class="content-desc">
                    Rincian Transaksi Per Semester
                </h5>
    ...
    @php
        if ($filters->semester == 'Ganjil') {
            $months = [null, 'Januari', 'Februari', 'Maret', 'April', 'Mei', 'Juni'];
        } else {
            $months = [null, 'Juli', 'Agustus', 'September', 'Oktober', 'November', 'Desember'];
        }
    @endphp
    ...
    @push('addon-scripts')
        ...
        <script>
            $(document).ready(function() {
                ...
                $.ajax({
                    method: "POST",
                    url: "{{ route('reports.getDiagramDataSemester') }}",
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                    },
                    data: {
                    ...
                    Highcharts.chart('lineChart', {
                        title: {
                            text: 'Laporan Transaksi Pembayaran Per Semester',
                            align: 'left'
                        },
                    ...
                    Highcharts.chart('pieChart2', {
                        ...
                        series: [{
                            ...
                            data: [{
                                ...
                                name: 'Uang Pemeliharaan & Pengembangan',
                                y: data.total_upp
                            }, {
                                name: 'Potongan Uang Pemeliharaan & Pengembangan',
                                y: data.total_potongan_upp
                            }, {
