        ...
        <div class="row">
            <div class="col-12">
                ...
                <h5 class="content-desc mb-4">
                    {{ __('Detail transaksi pembayaran uang sekolah untuk aplikasi SPP Al Ulum.') }}</h5>
            </div>
            @if ($errors->any())
            ...
            @if (Auth::user()->roles != 'User')
                <div class="col-12">
                    <a href="{{ route('discount.create') }}" class="btn btn-pink btn-radius mb-3" data-bs-toggle="modal"
                        data-bs-target="#transactionModal" data-bs-whatever="@getbootstrap">
                ...
                <div class="statistics-card card">
                    ...
                    <div class="row">
                        <div class="col-12 col-md-7">
                            <form method="post" action="{{ route('transactions.store') }}" id="selectForm">
                                ...
                                <select name="id" id="id" class="form-select" required autofocus
                                    data-placeholder="Pilih Siswa...">
                                    <option></option>
                                    @foreach ($list_siswa as $item)
                                        <option value="{{ $item->id }}"
                                            {{ $data->id == $item->id ? 'selected' : '' }}>{{ $item->nis }} -
                                            {{ $item->nama }}</option>
                                    @endforeach
                                </select>
                        ...
                        <div class="col-12 col-md-5">
                            ...
                            <div class="input-group mb-3">
                                <input type="number" class="form-control" min="0" id="tahun_masuk"
                                    value="{{ $data->tahun_angkatan }}" disabled placeholder="0">
                            </div>
                        </div>
                    </div>
                    @php
                    ...
                    <div class="row">
                        <div class="col-12 col-md-7">
                            ...
                            <div class="input-group mb-3">
                                <input type="text" class="form-control" id="tingkat" value="{{ $tingkat }}"
                                    disabled>
                            </div>
                        </div>
                        <div class="col-12 col-md-5">
                            ...
                            <div class="input-group mb-3">
                                <input type="text" class="form-control" id="kelas"
                                    value="{{ $data->kelas }} / {{ $data->grup }}" disabled>
                            </div>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-12 col-md-7">
                            ...
                            <select id="potongan_siswa" class="js-example-basic-multiple form-select"
                        ...
                        <div class="col-12 col-md-5">
                            ...
                            <div class="input-group mb-3">
                                <input type="text" class="form-control" id="tahun_masuk" value="@currency($schoolFeeAmount[$kelas])"
                                    ...
                                    @foreach ($months as $item)
                                        @if (\Carbon\Carbon::parse($item)->format('Y-m-d') < \Carbon\Carbon::now('Asia/Jakarta')->format('Y-m-d'))
                                            ...
                                            <option value="{{ $item }}" selected>
                                                {{ \Carbon\Carbon::parse($item)->format('M Y') }}</option>
                                        @endif
                                    @endforeach
                                </select>
                            @else
                            ...
                            <tbody>
                                ...
                                @foreach ($transactions as $item)
                                    ...
                                    <tr>
                                        <td>{{ $no++ }}</td>
                                        <td>
                                            <a href="{{ route('transactions.details', $item->id) }}"
                                        ...
                                        <td>
                                            @if ($item->status == 'Success')
                                                <a href="{{ route('transactions.print', $item->id) }}"
                                                    class="btn btn-secondary btn-sm" title="Cetak" target="_blank">
                        ...
                        <div class="row">
                                    ...
                                    <select name="tahun_ajaran" id="tahun_ajaran" class="form-select" required>
                                        @foreach ($schoolYear as $item)
                                            <option value="{{ $item->tahun_ajaran }}"
                                                {{ old('tahun_ajaran', $data->tahun_ajaran) == $item->id ? 'selected' : '' }}>
                            ...
                            <div class="col-12 col-md-2">
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control" id="nama"
                                        value="{{ $data->kelas }} / {{ $data->grup }}" disabled>
                                    <input type="hidden" class="form-control" name="tingkat"
                                        value="{{ $data->tingkat }}{{ $data->grup }}{{ $data->kelas }}" required>
                                </div>
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-12 col-md-6">
                                <div class="mb-3">
                                    ...
                                    <select name="bulan[]" id="bulan" class="form-select" multiple="multiple"
                                        data-placeholder="Pilih bulan...">
                                        @foreach ($months as $month)
                                            <option value="{{ $month }}"
                                                {{ old('bulan') == $month ? 'selected' : '' }}
                                                {{ $loop->index != 0 ? "disabled='disabled'" : '' }}>
                                                {{ \Carbon\Carbon::parse($month)->format('M Y') }}</option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-12 col-md-6">
                                <div class="mb-3">
                                    ...
                                    <input type="hidden" class="form-control" name="uang_sekolah"
                                        value="{{ $schoolFeeAmount[$kelas] }}" required>
                                    <input type="text" class="form-control" id="uang_sekolah"
                                        data-us="{{ $schoolFeeAmount[$kelas] }}" value="@currency($schoolFeeAmount[$kelas])" disabled>
                                </div>
                            </div>
                        </div>
                        <div class="row">
                        ...
                        <div class="row" id="containerLainnya">
                            <div class="col-12">
                                <div class="form-check form-switch form-check-inline">
                                    <input class="form-check-input" type="checkbox" role="switch" name="lainnya"
                                        id="lainnyaSwitch">
                                    <label for="total_lainnya" class="form-check-label">
                                        Pembayaran Lainnya :
                                        <button type="button" class="btn btn-secondary btn-sm" disabled
                                            id="btnLainnya">
                                            <img src="{{ url('/assets/img/global/plus-circle.svg') }}" width="20"
                                                alt="">
                                        </button>
                                    </label>
                                </div>
        ...
        <script>
            $(document).ready(function() {
                ...
                $('#btnLainnya').click(function(e) {
                    e.preventDefault();
                    if (x < 3) {
                        $('#containerLainnya').append('<div class="row" id="element' + x +
